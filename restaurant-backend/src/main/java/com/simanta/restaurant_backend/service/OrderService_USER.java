...
public class OrderService_USER { 
    ...
    public Order_PostOrder_response_USER_DTO placedOrder(final User user,final Order_PostOrder_request_USER_DTO order_PostOrder_request_USER_DTO) throws MessagingException{
        ...
        final Address address = addressRepository.findByIdAndUser(order_PostOrder_request_USER_DTO.getAddressId(),user)
                .orElseThrow(()-> new OrderService_USER_Exception("Address not found"));
        ...
        for(CartItem cartItem:cart.getCartItem()){                 
            ...
            double total = cartItem.getQuantity() * cartItem.getMenu().getPrice();
        ...
        double finaltotalprice = totalprice + deliveryCharge;
        ...
        return new Order_PostOrder_response_USER_DTO("Order Placed.",order.getId());
    }

    
    // Get All Order
    public Order_Get_All_By_User_response_DTO get_All_Order(final String email){

        User user = authRepository.findByEmail(email)
                .orElseThrow(()-> new OrderService_USER_Exception("User not found"));

        Order order = orderRepository.findByUser(user)
            .orElseThrow(()-> new OrderService_USER_Exception("Order not found"));

        List<OrderItem_Get_All_By_User_response_DTO> orderitems = new ArrayList<>();

        double totalprice = 0;

        for(OrderItem orderItem:order.getOrderitems()){

            OrderItem_Get_All_By_User_response_DTO orderitem = new OrderItem_Get_All_By_User_response_DTO(
                orderItem.getOrder().getId(),orderItem.getMenu().getId(),orderItem.getQuantity(),orderItem.getPrice());
        ...
        return new Order_Get_All_By_User_response_DTO(order.getId(), user.getId(), orderitems,order.getSelectdefaultaddress().getId(), order.getOrderStatus(), totalprice);
    }

    
    // Get Order by id
    public Order_Get_By_Id_response_DTO get_order_By_id(final Long orderid){

        final Order order = orderRepository.findById(orderid) 
            .orElseThrow(()-> new OrderService_USER_Exception("Order not found"));

        List<Orderitem_Get_By_Id_response_DTO> orderitems = new ArrayList<>();

        double totalprice = 0;

        for(OrderItem orderItem : order.getOrderitems()){

            Orderitem_Get_By_Id_response_DTO orderitem = new Orderitem_Get_By_Id_response_DTO(orderItem.getOrder().getId(),
                 orderItem.getMenu().getId(),orderItem.getQuantity(), orderItem.getPrice());
        ...
        Address address = order.getSelectdefaultaddress();

        Address_View_response_USER_DTO address_View_response = new Address_View_response_USER_DTO(address.getId(), address.getFullName(), address.getPhoneNumber(), address.getStreet(), address.getArea(), 
            address.getCity(), address.getState(), address.getPincode(), address.getCountry(), address.isDefault(), address.getAddressType());

        return new Order_Get_By_Id_response_DTO(order.getId(), order.getUser().getId(), orderitems, address_View_response,order.getOrderStatus(),totalprice);
    }
    ...
    public Order_Cancel_response_USER_DTO cancel_order(final Long orderId,final Long userId) throws MessagingException{
        ...
        if(order.getOrderStatus() == OrderStatus.CANCELLED){
            return new Order_Cancel_response_USER_DTO("Order already canceled.");
        }

        if(order.getOrderStatus() == OrderStatus.CONFIRMED){
            return new Order_Cancel_response_USER_DTO("Order is already confirmed, cannot cancel.");
        }

        if(order.getOrderStatus() == OrderStatus.DELIVERED){
            return new Order_Cancel_response_USER_DTO("Order is delivered cannot Cancel.");
        }
 
        order.setOrderStatus(OrderStatus.CANCELLED);

        orderRepository.save(order);
        email_for_OrderCancel.message_Sending_for_OrderCancel(order.getUser().getEmail(), orderId);

        return new Order_Cancel_response_USER_DTO("Your order has been Canceled.");
    }
